...
// Core Processing Edge Logic
app.post('/tools/json', async (c) => {
  const body = await c.req.parseBody()
  const rawInput = body.input || ''
  const spaceSetting = body.indent || '2'
  
  if (!rawInput.trim()) {
    return c.html(<JsonFormatterView input={rawInput} error="Input configuration cannot be empty." />)
  }
  ...
  if (spaceSetting === 'tab') {
    ...
    indent = parseInt(spaceSetting, 10);
  }

  try {
    // Edge computation: Parse string to native object, format cleanly back to string
    const parsedObj = JSON.parse(rawInput)
    const formattedOutput = JSON.stringify(parsedObj, null, indent)
    
    return c.html(<JsonFormatterView input={rawInput} output={formattedOutput} indent={spaceSetting} />)
  } catch (err) {
    // Catch validation error details natively generated by the V8 engine
    return c.html(<JsonFormatterView input={rawInput} error={err.message} indent={spaceSetting} />)
  }
})
